...
for z = 1:length(V_sens_in2)
    veh_z = V_sens_in2(z);
    pos_veh_z = veh_z.Position;
    d_z_in2 = dist(pos_veh_z(1),pos_veh_z(2),X_in2,Y_in2);
    t_z_in2 = d_z_in2/v;
    cond = t_z_in2 - t_in2;
    %pentru in2 corespunde t_wait_in2
    if(t_wait_in2 == 0)
        if cond >= t_safety || t_safety <= -(cond)
            t_wait_in2 = 0;
        elseif t_safety >= -(cond)
            t_wait_in2 = t_safety + cond;
            t_wait_in2 = max(0,t_wait_in2);
        end
    else
        % cond1 = cond - t_wait_in2;
         if cond >= t_safety || t_safety <= -(cond)
             t_wait_in2 = t_wait_in2;
         elseif t_safety >= -(cond)
            t_wait_in2 = t_wait_in2 + t_safety + cond;
         end
    end
end
...
for z=1:length(V)
    ...
    switch in
        case 1
            j = 1;
            if ismember(veh_z,V_in2)
                exp = t_in1_in2 + t_in1 + t_wait_in1;
                cond = exp - t_in2;
                conditie = cond - t_wait_in2
        ...
        case 3
            ...
            if ismember(veh_z,V_in2)
                ...
                if conditie >= t_safety || t_safety <= -conditie
                    t_wait_in2 = t_wait_in2;
                elseif t_safety >= -conditie
                    t_wait_in2 = t_safety + conditie;
                end
            end
        case 4
            j = 4;
            if ismember(veh_z,V_in2)
                exp = t_in4_in2 + t_in4 + t_wait_in4;
                cond = exp - t_in2;
                conditie = cond - t_wait_in2
                if conditie >= t_safety || t_safety <= -conditie
                    t_wait_in2 = t_wait_in2;
