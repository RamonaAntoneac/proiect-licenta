...
for z = 1:length(V_sens_in1)
    veh_z = V_sens_in1(z);
    pos_veh_z = veh_z.Position;
    d_z_in1 = dist(pos_veh_z(1),pos_veh_z(2),X_in1,Y_in1);
    t_z_in1 = d_z_in1/v;
    cond = t_z_in1 - t_in1;
    %pentru in1 corespunde t_wait_in1
    if(t_wait_in1 == 0)
        if cond >= t_safety || t_safety <= -(cond)
            t_wait_in1 = 0;
        elseif t_safety >= -(cond)
            t_wait_in1 = t_safety + cond;
            t_wait_in1 = max(0,t_wait_in1);
        end
    else
        if cond >= t_safety || t_safety <= -(cond)
            t_wait_in1 = t_wait_in1;
        elseif t_safety >= -(cond)
            t_wait_in1 = t_wait_in1 + t_safety + cond;
        end
    end
end
...
for z=1:length(V)
    ...
    switch in
        ...
        case 2
            j = 2;
            if ismember(veh_z,V_in1)
                exp = t_in2_in1 + t_in2 + t_wait_in2;
                cond = exp - t_in1;
                conditie = cond - t_wait_in1
                if conditie >= t_safety || t_safety <= -conditie
                    t_wait_in1 = t_wait_in1;
                elseif t_safety >= -conditie
                    t_wait_in1 = t_safety + conditie;
                end
            end
        case 3
            j = 3;
            if ismember(veh_z,V_in1)
                exp = t_in3_in1 + t_in3 + t_wait_in3;
                cond = exp - t_in1;
                conditie = cond - t_wait_in1
                if conditie >= t_safety || t_safety <= -conditie
                    t_wait_in1 = t_wait_in1;
        ...
        case 4
            j = 4;
            if ismember(veh_z,V_in1)
                exp = t_in4_in1 + t_in4 + t_wait_in4;
                cond = exp - t_in1;
                conditie = cond - t_wait_in1
                if conditie >= t_safety || t_safety <= -conditie
                    t_wait_in1 = t_wait_in1;
